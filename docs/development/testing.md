...
92 tests across seven areas; `python -m pytest` runs the whole suite in a few
...
| gradient flow | all trainable parameters receive gradients; checkpointing preserves loss |
...
| generation | greedy/seeded determinism, max-new-tokens, stop sequences, stream-deltas ≡ full text, long-prompt truncation |
